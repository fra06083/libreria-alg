...
public class Prim<D> implements MST<D> {
	...
	public Graph<D> MinimumSpanningTree(Graph<D> graph) {
		...
		DHeap<Double, Vertex<D>> pq = new DHeap<>(); //struttura priority queue pq
		//mappa da vertici a corrispondenti nodi in pq
		Map<Vertex<D>, PriorityQueueNode<Double,Vertex<D>>> pqnodes = new HashMap<Vertex<D>, PriorityQueueNode<Double,Vertex<D>>>();
		Map<Vertex<D>, Boolean> bool = new HashMap<Vertex<D>, Boolean>(); //flag che indica vertice già in MST
		Map<Vertex<D>, Double> distance = new HashMap<Vertex<D>, Double>(); //distanza dei vertici dall'MST
		Map<Vertex<D>, Vertex<D>> parent = new HashMap<Vertex<D>, Vertex<D>>(); //parent dei vertici
		ArrayList<Vertex<D>> vert = graph.vertexes(); //estrae i vertici dal grafo
		for (Vertex<D> v : vert) { //per ogni vertice del grafo
		...
		while (!pq.isEmpty()) { //finché ci sono vertici nella coda
			...
			pq.deleteMin(); //estrae da pq il vertice più vicino all'MST
