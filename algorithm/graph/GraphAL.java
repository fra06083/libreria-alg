...
public class GraphAL<D> implements Graph<D> {
	...
	public Edge<D> areAdjacent(Vertex<D> x, Vertex<D> y) {
		...
		Iterator<EdgeAL<D>> it = xAL.adjac.iterator();
		while (it.hasNext()) {
			if (it.next().dest == y) {
				return it.next();
			}
	...
	public void addEdge(Vertex<D> source, Vertex<D> dest) {
		VertexAL<D> eSourceAL = (VertexAL<D>) source;
		VertexAL<D> eDestAL = (VertexAL<D>) dest;
		EdgeAL<D> e = new EdgeAL<D>(eSourceAL, eDestAL);
		eSourceAL.adjac.add(e);
		m++;
	...
	public void addEdge(Vertex<D> source, Vertex<D> dest, double weight) {
		VertexAL<D> eSourceAL = (VertexAL<D>) source;
		VertexAL<D> eDestAL = (VertexAL<D>) dest;
		EdgeAL<D> e = new EdgeAL<D>(eSourceAL, eDestAL, weight);
		eSourceAL.adjac.add(e);
		m++;
	...
	public void removeEdge(Edge<D> e) {
		EdgeAL<D> eAL = (EdgeAL<D>)e;
		VertexAL<D> vAL = eAL.source;
		...
		// se arriva qua non è stato trovato
